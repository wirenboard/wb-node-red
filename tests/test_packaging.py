...
def test_helper_postinst_does_not_provision_mqtt():
    # Lazy/opt-in provisioning: the helper touches no host state at its own
    # install — the first bridge service that needs the broker triggers it.
    postinst = (HELPER / "debian" / "postinst").read_text()
    assert "provision-mqtt" not in postinst
    assert "wb_docker_app_provision_mqtt" not in postinst


def test_node_red_postinst_provisions_the_broker_before_install():
    # Node-RED is a bridge service: it provisions the broker bridge, THEN
    # installs, passing its container uid (1000) so the helper chowns /data.
    postinst = (SERVICE / "debian" / "postinst").read_text()
    assert "wb_docker_app_provision_mqtt" in postinst
    assert "wb_docker_app_install node-red 1000" in postinst
    assert (postinst.index("wb_docker_app_provision_mqtt")
            < postinst.index("wb_docker_app_install"))


def test_node_red_postrm_reloads_proxy_on_removal_only():
    postrm = (SERVICE / "debian" / "postrm").read_text()
    assert "wb_docker_app_reload_proxy" in postrm
    assert "remove | purge)" in postrm
    # not on upgrade — the new install reloads its own fresh server block
    assert "upgrade" in postrm
