...
> **Обновление 2026-06 — часть решений пересмотрена.** Проект переведён на
> подход *minimal-first* (тонкий helper + статические сервисы). Актуальные
> решения — в `docs/adr/0003` (minimal-first) и `docs/adr/0004` (MQTT
> boot-timing через `ip_nonlocal_bind`), `docs/adr/0005` (reverse-proxy + auth, по итогам валидации на железе) и `docs/adr/0006` (Node-RED → mirror + вендоренный pure-JS палет, derived НЕ нужен — проверено на armv7l). Секции про генерацию nginx, аллокацию
> внутренних портов, `descriptor`/labels-контракт, wb-diag-collect и
> `After=docker.service`, derived-образ для Node-RED (§3.4, §3.5.1, §3.7, §3.8, §3.9, §3.10, §3.11.1) **устарели** —
> читать как исторический контекст, источник истины теперь ADR.
